<script lang="ts">
  ...
  interface Props {
    ...
    onClose: () => void;
    chatId: string | null;
    onFetchMembers: (chatId: string) => Promise<RenderedMember[]>;
    onKickMember: (chatId: string, userId: number, name: string) => Promise<void>;
    onToggleAdmin: (chatId: string, userId: number, promote: boolean, name: string) => Promise<void>;
  }
  ...
  const loadMembers = async () => {
    ...
    try {
      ...
      errorMessage = error?.message || 'Failed to load members. Bot may not have admin rights.';
    } finally {
      isLoading = false;
    }
  };
        ...
        {#if isLoading}
          <div class="flex items-center justify-center py-12">
            <div class="text-center">
              <div class="mb-3 h-8 w-8 animate-spin rounded-full border-4 border-slate-600 border-t-blue-500 mx-auto"></div>
        ...
        {:else}
          <div class="space-y-2">
            {#each members as member (member.id)}
              <div class="flex items-center justify-between rounded-lg border border-slate-800 bg-slate-800/40 p-3 hover:bg-slate-800/60 transition">
                <div class="flex-1">
                ...
                {#if member.status !== 'creator'}
                  <div class="flex items-center gap-1 ml-3">
                    <button
                      ...
                      title={member.status === 'administrator' ? 'Demote' : 'Promote'}
                      aria-label={member.status === 'administrator' ? 'Demote member' : 'Promote member'}
                    >
          ...
          <div class="mt-4 text-xs text-slate-500 text-center border-t border-slate-800 pt-3">
