...
<div
  class="flex flex-col gap-6 bg-slate-900 px-6 py-6 text-slate-100 max-h-[80vh] overflow-y-auto"
>
  {#if botInfo}
    <section
      class="rounded-xl border border-slate-800 bg-slate-900/80 p-5 shadow-inner"
    >
      <h2 class="text-lg font-semibold text-white">Bot information</h2>
      <dl
        class="mt-4 grid grid-cols-1 gap-4 text-sm text-slate-300 sm:grid-cols-2"
      >
        <div>
  ...
  >
    ...
    <p class="mt-2 text-sm text-slate-400">
      Update your bot token (from BotFather). The app will reload with the new
      token.
    </p>
    ...
    <div class="mt-4 flex flex-col gap-3 sm:flex-row">
      <input
        ...
        bind:value={newToken}
        onfocus={() => (passwordInputType = "text")}
      />
  ...
  >
    ...
    <p class="mt-2 text-sm text-slate-400">
      Optional CORS proxy prefix (e.g., https://cors.isomorphic-git.org/). Leave
      empty to use direct Telegram API.
    </p>
    ...
    <div class="mt-3 text-xs text-slate-500">
      Changes take effect immediately. The bot will reinitialize with the new
      proxy settings.
    </div>
  </section>
  ...
  >
    ...
    <p class="mt-2 text-sm text-slate-400">
      If you receive error 409 when polling, the webhook might still be active.
      Delete it below.
    </p>
  ...
  >
    ...
    <p class="mt-2 text-sm text-slate-400">
      Removes local chat history stored for this bot token. This action cannot
      be undone.
    </p>
