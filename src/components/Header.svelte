<script lang="ts">
  ...
  import { Menu, Settings2, Users2 } from "lucide-svelte";
  ...
  const botDescription = $derived(() => {
    ...
    const username = botInfo.username ? `@${botInfo.username}` : botInfo.first_name;
    return `${username} • ID ${botInfo.id}`;
  });
  const statusVariant = $derived(() => {
    const text = statusText?.toLowerCase() ?? "";
    if (text.includes("token")) return "bg-slate-500";
    if (text.includes("connected") && !text.includes("disconnected")) return "bg-emerald-500";
    if (text.includes("disconnected") || text.includes("error")) return "bg-rose-500";
    return "bg-amber-400";
  });
</script>
...
<header class="flex items-center justify-between border-b border-slate-800 bg-slate-900 px-4 py-3">
  <div class="flex min-w-0 items-center gap-3">
    ...
    <div class="flex h-10 w-10 items-center justify-center rounded-full bg-slate-800 text-sm font-semibold uppercase text-slate-100">
      {displayInitials}
    </div>
    ...
    <div class="flex min-w-0 flex-col">
      ...
      <div class="mt-1 flex items-center gap-2 text-xs text-slate-400">
        <span class={`inline-flex h-2 w-2 rounded-full ${statusVariant}`} aria-hidden="true"></span>
  ...
  <div class="flex items-center gap-2 md:gap-3">
    ...
    <div class="hidden min-w-0 flex-col text-right text-xs text-slate-400 lg:flex">
