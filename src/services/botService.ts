...
export class BotService {
  ...
  private latestUpdateCallback: ((updates: TelegramUpdate[]) => void) | null =
    null;
  ...
  setGatewayMode(mode: GatewayMode) {
    ...
    if (this.config && wasRunning) {
      void this.start(
        this.latestUpdateCallback || undefined,
        this.latestStatusCallback || undefined,
      );
    }
  }
  ...
  setConfig(config: BotConfig) {
    ...
    if (this.mode === "mtproto") {
      mtProtoGateway
        .configure({
          apiId: config.apiId || 4,
          apiHash: config.apiHash || "014b35b6184100b085b0d0572f9b5103",
        ...
        .catch((error) => {
          ...
          if (this.statusCallback) {
            this.statusCallback(
              "error",
              error?.message || "MTProto configure failed",
            );
          }
        });
      return;
  ...
  private async callBotApiJson<T = any>(method: string, payload: Record<string, unknown> = {}): Promise<{
    ok: boolean;
    result?: T;
    description?: string;
    error_code?: number;
    ...
    try {
      const apiRoot = this.config.proxyPrefix
        ? this.config.proxyPrefix.replace(/\/+$/, "")
        : "https://api.telegram.org";
      const response = await fetch(`${apiRoot}/bot${this.config.token}/${method}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      return {
        ok: false,
        description: error?.message || `Failed to call Bot API ${method}`,
        error_code: error?.error_code || 0,
      };
    }
  }
  ...
  async getBotApiMe() {
    return this.callBotApiJson<any>("getMe");
  }

  async getBotApiChat(chatId: number | string) {
    return this.callBotApiJson<any>("getChat", { chat_id: chatId });
  }

  async getBotApiChatMember(chatId: number | string, userId: number) {
    return this.callBotApiJson<any>("getChatMember", {
      chat_id: chatId,
      user_id: userId,
    });
  }
  ...
  async sendMediaGroup(
    ...
    if (files.length < 2) {
      return {
        ok: false,
        description: "Media group requires at least 2 files",
      };
    }
  ...
  /** Fetch a user's profile photos through Bot API regardless of realtime gateway. */
  async getBotApiUserProfilePhotos(userId: number, limit = 1): Promise<{
    ok: boolean;
    ...
    try {
      const apiRoot = this.config.proxyPrefix
        ...
        : "https://api.telegram.org";
      const response = await fetch(
        `${apiRoot}/bot${this.config.token}/getUserProfilePhotos`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      return {
        ok: false,
        description: error?.message || "Failed to get user profile photos",
        error_code: error?.error_code || 0,
      };
    }
  }
  ...
  async getChatMember(chatId: number | string, userId: number) {
    if (this.mode === "mtproto") {
      return {
        ok: false,
        description:
          "Chat permissions are currently available through the Bot API gateway only",
      };
    }
    if (!this.bot) throw new Error("Bot not initialized");
  ...
  async setMyProfilePhoto(photo: File) {
    if (this.mode === "mtproto") {
      return {
        ok: false,
        description:
          "Changing the bot profile photo is currently available through the Bot API gateway only",
      };
    }
  ...
  async setChatPhoto(chatId: number | string, photo: File) {
    if (this.mode === "mtproto") {
      return {
        ok: false,
        description:
          "Changing chat photos is currently available through the Bot API gateway only",
      };
    }
    if (!this.bot) throw new Error("Bot not initialized");
  ...
  async setMyDescription(description?: string) {
    ...
    try {
      const result = await this.bot.api.setMyDescription(description || "");
      return { ok: true, result };
    } catch (error) {
      return {
        ok: false,
  ...
  async start(
    ...
    if (this.mode === "mtproto") {
      ...
      try {
        ...
        if (statusCallback)
          statusCallback("error", error?.message || "MTProto failed");
      }
      return;
    }
    ...
    console.debug("[BotService] Starting grammy polling with bot.start()...");
